...
p6<- ggplot(df, aes(df$absences)) + geom_histogram(binwidth=2,color="darkblue",fill="darkblue")+labs(x="Abscences")
...
fitbin <- glm(pass~ G1 + G2,data=dfbin,family="binomial")
summary(fitbin)
...
```{r}
fitbin2= glm(pass~Fedu+ famrel+ goout+ Walc+ G1 + G2,data=train,family="binomial")
```
...
```{r}
rf_model <- randomForest(pass ~ Fedu+ famrel+ goout+ Walc+ G1+  G2, data = train, ntree = 4)
...
round(bhat(por, formula=f), 3)
```

```{r}
bhat.fit= boot(por, statistic=bhat, R=2000, formula=f)
dim(bhat.fit$t)
...
```{r}
fitperm1 <- lmp(G3~age +famrel+ absences+ G1+ G2,data=por)
summary(fitperm1)
summary(fit1)
```
...
```{r}
fitpoi <- glm(G3~failures+ schoolsup+ famrel+ absences+ G1+ G2,family=poisson(),por)
...
Compare coef to permutation test
```{r}
fitperm2 <- lmp(G3~failures+ schoolsup+ famrel+ absences+ G1+ G2,family=poisson(),por)
summary(fitperm2)
summary(fitpoi)
```
