...
#pibintestcv= predict(fitbin3cv, type='response')
...
library(caret)
set.seed(123)
rfcv_model <- train(pass ~ Fedu+ famrel+ goout+ Walc+ G2, data = train, method = "rf", trControl = trainControl(method = "cv", number = 10))
rfcv_pred_prob <- predict(rfcv_model, newdata = test, type = "prob")
rfcv_pred <- rfcv_pred_prob[,2]
table(rfcv_pred > 0.5, test$pass)


misclas3= c(cost_misclass(test$pass, pibintestcv), cost_misclass(test$pass, rf_pred), cost_misclass(test$pass, rfcv_pred))
names(misclas3)= c('glm - binomial','random forest', 'random forest - cv')
misclas3
...
```
 ...
 what if we change it by doing a binary distinction of students that improve?
