...
```{r}
por <- read.csv2('~/github/MSC-Final-Project/data/student-por.csv')
```
...
summary(fitallpnog)
```
#Full QUASI-POISSON adjusting for overdispersion
```{r}
fitallqp <- glm(G3~.,data=por,family="quasipoisson")
summary(fitallqp)
```
#Full quasi-poisson without grades
```{r}
...
dfpass <- select(dfpass, -c("pass", "resl", "predl", "gradelevel", "gradecat"))
dffail <- subset(df, pass==0)
dffail <- select(dffail, -c("pass", "resl", "predl", "gradelevel", "gradecat"))
```
...
summary(fitallpass)
```
#Best BIC for students who pass
```{r}
...
#Full linear model for students who fail
```{r}
fitallfail <- lm(G3~.,data=dffail)
summary(fitallfail)
```
#Best BIC for students who fail
```{r}
